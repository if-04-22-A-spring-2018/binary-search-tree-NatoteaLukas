...
void add(Bst* bst, int value)
{
  ...
  {
    if (node->left_subtree == 0) {
      Bst new_bst = (Node*)malloc(sizeof(struct Node));
      new_bst->data = value;
      new_bst->left_subtree = 0;
      new_bst->right_subtree = 0;
      node->left_subtree = new_bst;
      node->depth++;
      bst = &node;
    }
    else
      add(&node->left_subtree, value);
  }
  else
  {
    if(node->right_subtree == 0)
    {
      Bst new_bst = (Node*)malloc(sizeof(struct Node));
      new_bst->data = value;
      new_bst->left_subtree = 0;
      new_bst->right_subtree = 0;
      node->right_subtree = new_bst;
      node->depth++;
      bst = &node;
    }
    else
      add(&node->right_subtree, value);
  }
}
